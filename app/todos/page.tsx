    ...
    <main className="min-h-screen pt-32 pb-16 px-4 bg-slate-50/30">
      <div className="container mx-auto max-w-4xl">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <h1 className="text-4xl font-oswald text-slate-900 mb-2 font-bold uppercase tracking-tight">Liste des Tâches</h1>
            <p className="text-slate-500 text-lg">Gérez vos interventions et le suivi des chantiers en temps réel.</p>
          </div>
          <button 
            onClick={fetchTodos}
        ...
        {loading && !todos.length ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50">
            ...
            <p className="text-slate-500 font-medium">Chargement des données en cours...</p>
          </div>
        ) : error ? (
          <div className="bg-red-50 text-red-700 p-8 rounded-3xl border border-red-100 shadow-xl shadow-red-200/20">
            <div className="flex items-center gap-4 mb-4 text-red-600">
              <AlertTriangle className="w-8 h-8" />
              <h3 className="font-bold text-xl uppercase tracking-tight">Dysfonctionnement du service</h3>
            </div>
            <p className="mb-6 text-slate-600 leading-relaxed">
              Une erreur est survenue lors de la tentative de connexion à nos serveurs de données. Cela peut provenir d'une maintenance ou d'un problème réseau.
            </p>
            <div className="bg-white/80 p-4 rounded-xl border border-red-200 text-xs font-mono text-red-500">
              <strong>Rapport d'erreur Supabase :</strong> {error.message || "Impossible de décoder l'erreur."}
            </div>
            <button 
              onClick={fetchTodos}
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/60 border border-slate-100 overflow-hidden">
            <div className="p-8 border-b border-slate-50 bg-slate-50/50">
               <div className="flex items-center gap-4">
                  ...
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Dernière mise à jour: {new Date().toLocaleTimeString()}</div>
               </div>
            </div>
            ...
            <div className="p-4 md:p-8">
              {todos && todos.length > 0 ? (
                    ...
                    >
                      <div className="flex items-start gap-5">
                        ...
                        <div className="min-w-0">
                          <h3 className={`font-bold text-lg leading-tight transition-all ${todo.is_completed ? 'text-slate-400 line-through' : 'text-slate-900'}`}>
                            {todo.title}
                          </h3>
                          {todo.description && (
                            <p className={`text-sm mt-1 transition-all ${todo.is_completed ? 'text-slate-400' : 'text-slate-500'}`}>
                              {todo.description}
                            </p>
                          )}
                          <div className="flex items-center gap-3 mt-3">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">ID: {todo.id.substring(0, 6)}</span>
                            {todo.created_at && (
                      ...
                      <div className="md:shrink-0">
                         <span className={`inline-flex items-center justify-center px-6 py-2 rounded-full text-xs font-black uppercase tracking-tighter transition-all ${todo.is_completed ? 'bg-emerald-100/50 text-emerald-600 border border-emerald-100' : 'bg-blue-600 text-white shadow-lg shadow-blue-600/20'}`}>
                ...
                <div className="text-center py-16">
                  ...
                  <h3 className="text-xl font-bold text-slate-900 mb-2 whitespace-nowrap">Tout est en ordre !</h3>
                  <p className="text-slate-500 max-w-xs mx-auto">Toutes vos tâches ont été traitées ou aucune n'a été planifiée.</p>
                </div>
              )}
            </div>
          </div>
