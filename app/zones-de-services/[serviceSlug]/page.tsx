...
export default async function ServiceZonesPage({ params }: { params: Promise<{ serviceSlug: string }> }) {
  const resolvedParams = await params;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16">
          ...
          <p className="text-xl text-slate-400 max-w-2xl leading-relaxed">
            Nous intervenons en moins de 30 minutes dans toutes les communes listées ci-dessous pour vos besoins en {serviceInfo.title.toLowerCase()}.
          </p>
        </div>
            ...
            return (
              <div key={province} className="bg-white/5 border border-white/10 rounded-3xl p-8 hover:bg-white/10 transition-colors group">
                <h2 className="text-2xl font-black mb-6 flex items-center justify-between">
                  {province}
                  <span className="text-xs bg-white/10 px-2.5 py-1 rounded-full font-mono text-slate-400">{cities.length}</span>
                </h2>
