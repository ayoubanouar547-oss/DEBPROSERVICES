...
export function PageHero({
  ...
  // Define badges colors mapping to match the original style
  const badgeColors = [
    ...
    <section className="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 px-4 sm:px-6 lg:px-12 py-10 pt-32 min-h-[90vh] items-center">
      <div className="col-span-1 lg:col-span-7 flex flex-col justify-center">
        {badges && badges.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {badges.map((badge, idx) => (
              <span key={idx} className={`text-[10px] font-bold px-2 py-1 rounded border uppercase tracking-widest ${badgeColors[idx % badgeColors.length]}`}>
                {badge}
              </span>
            ))}
          </div>
        ...
        <div className="space-y-4">
          <h1 className="font-black leading-[1.1] mb-6 pt-4 text-4xl md:text-5xl lg:text-7xl text-white flex flex-wrap items-center gap-x-4 gap-y-2">
            <span>
              {title}
            </span>
            {titleHighlight && (
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300 text-shiny">
                {titleHighlight}
              </span>
            )}
          </h1>
